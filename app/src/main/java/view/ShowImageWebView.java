...
public class ShowImageWebView extends WebView {
    ...
    private List<String> getAllImageUrlFormSrcObject(List<String> listImageUrl) {
        for (String image : listImageUrl) {
            ...
            while (matcher.find()) {
                String iamgeURL = matcher.group().substring(0, matcher.group().length() - 1);
                int imageLen = iamgeURL.length();
                if(imageLen == 46 && iamgeURL.contains("large"))
                listImgSrc.add(iamgeURL);
            }
        }
        return listImgSrc;
    }
